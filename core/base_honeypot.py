...
from honeybash.logging.event_logger import set_up_error_logging, setup_logger
from honeybash.utils.network_utils import get_free_port, service_has_started


class BaseServer(ABC):
    """Base Honeypot Server class for HoneyBash framework."""
    ...
    def __init__(self, **kwargs):
        ...
        self.process = None
        self.uuid = f"honeybashlogger_{self.__class__.__name__}_{str(uuid4())[:8]}"
        ...
        self.ip = kwargs.get("ip") or (hasattr(self, "ip") and self.ip) or "0.0.0.0"
        self.port = (
            (kwargs.get("port") and int(kwargs.get("port")))
    ...
    def _start_server(self) -> str:
        ...
        if service_has_started(int(self.port)):
            return "success"
        self.logger.error(f"HoneyBash Server {self.NAME} did not start")
        return "error"
...
# Alias for HoneyBash architectural structure
BaseHoneypot = BaseServer
QBaseServer = BaseServer
