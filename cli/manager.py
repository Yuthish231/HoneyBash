...
try:
    from signal import alarm, SIGALRM, SIGTSTP
except ImportError:
    SIGALRM = None
    SIGTSTP = None
...
try:
    ...
    def geteuid():
        return -1  # non-root on Windows


try:
    from netifaces import AF_INET, AF_LINK, ifaddresses, interfaces
except ImportError:
    from netifaces2 import AF_INET, AF_LINK, ifaddresses, interfaces
from psutil import net_io_counters, Process

from honeybash import (
    QDHCPServer,
    QDNSServer,
    QFTPServer,
    QHTTPProxyServer,
...
from honeybash.cli.banner import print_banner
from honeybash.logging import logger, setup_logger
from honeybash.utils.network_utils import clean_all, is_privileged
...
class SignalFence:
    def __init__(self, signals_to_listen_on, interval=1):
        ...
        for signal_to_listen_on in signals_to_listen_on:
            if signal_to_listen_on is not None:
...
class Termination:
    ...
    def await_termination(self):
        if self.strategy == "input":
            ...
            SignalFence([s for s in [SIGTERM, SIGINT, SIGTSTP] if s is not None]).wait_on_fence()
        else:
...
class HoneyBashManager:
    ...
    def main(self):
        print_banner()
        logger.info("HoneyBash Framework — For updates, visit https://github.com/honeybash")
        ...
        if self.options.list:
            print("Available HoneyBash Service Honeypots:")
    ...
    def _set_up_honeypots(self):  # noqa: C901
        ...
        if self.options.termination_strategy == "input":
            logger.info("Use [Enter] to exit or python -m honeybash --kill")
        ...
        if len(self.honeypots) > 0:
            ...
            if len(running_honeypots["good"]) > 0:
                ...
                logger.info(f"HoneyBash servers [{running_servers}] running...")

            if len(running_honeypots["bad"]) > 0:
                not_running_servers = ", ".join(running_honeypots["bad"])
                logger.warning(f"HoneyBash servers [{not_running_servers}] failed to start...")

            if len(running_honeypots["bad"]) == 0:
                logger.info("All configured HoneyBash servers started successfully!")
    ...
    def _start_all_servers(self):
        try:
            ...
            logger.exception(f"Starting HoneyBash honeypots failed: {error}")
    ...
    def _stop_servers(self):
        logger.info("[x] Stopping HoneyBash servers...")
        ...
        logger.info("[x] Please wait a few seconds...")
        sleep(2)

    def _start_chameleon_mode(self):  # noqa: C901,PLR0912
        logger.info("[x] HoneyBash Advanced Multi-Service Mode")
        ...
        if not self.options.test:
            logger.info("[x] HoneyBash deployment successful!")
            self._stats_loop(logs)
        else:
            self._stop_servers()

    def _setup_logging(self) -> logging.Logger:
        uuid = f"honeybashlogger_main_{str(uuid4())[:8]}"
    ...
    def _start_sniffer(self, sniffer_filter, sniffer_interface):
        logger.info("[x] Starting network sniffer")
        sniffer = QSniffer(
...
def _exit_handler():
    logger.info("[x] HoneyBash Cleanup Handler")
    clean_all()
    sleep(1)
...
def _parse_args() -> tuple[Namespace, dict[str, str | int]]:
    arg_parser = _ArgumentParser(
        description=(
            "HoneyBash — Modular honeypot framework supporting 14 network service honeypots "
            "with configurable logging and storage backends."
        ),
        usage=SUPPRESS,
    )
    ...
    arg_parser_setup.add_argument(
        "--setup",
        help="Target honeypot service, e.g., ssh or multiple services, e.g., ssh,http,https",
        metavar="",
        default="",
    )
    arg_parser_setup.add_argument(
    ...
    arg_parser_setup.add_argument("--kill", action="store_true", help="Kill all active honeypots")
    arg_parser_setup.add_argument("--verbose", action="store_true", help="Print verbose error messages")
    arg_parser_optional = arg_parser.add_argument_group("Honeypots options")
    arg_parser_optional.add_argument("--ip", help="Override binding IP", metavar="", default="")
    arg_parser_optional.add_argument(
        "--port",
        help="Override binding port (Do not use with multiple services!)",
        metavar="",
        default="",
    )
    arg_parser_optional.add_argument(
        "--username", help="Override default username", metavar="", default=""
    )
    arg_parser_optional.add_argument(
        "--password", help="Override default password", metavar="", default=""
    )
    arg_parser_optional.add_argument(
        "--config",
        help="Use a JSON configuration file for honeypot settings",
        metavar="",
        default="",
    )
    arg_parser_optional.add_argument(
        "--options", type=str, help="Extra operational options", metavar="", default=""
    )
    arg_parser_optional_2 = arg_parser.add_argument_group("General options")
    arg_parser_optional_2.add_argument(
        "--termination-strategy",
        help="Determines the strategy to terminate by (input or signal)",
        default="input",
        choices=["input", "signal"],
    )
    arg_parser_optional_2.add_argument("--test", action="store_true", help="Test a honeypot service")
    arg_parser_optional_2.add_argument(
        "--auto", help="Setup honeypots with random available ports", action="store_true"
    )
    arg_parser_chameleon = arg_parser.add_argument_group("Advanced Mode")
    arg_parser_chameleon.add_argument(
        "--chameleon", action="store_true", help="Enable multi-service honeypot deployment mode"
    )
    arg_parser_chameleon.add_argument(
        "--sniffer",
        action="store_true",
        help="Enable network sniffer",
    )
    arg_parser_chameleon.add_argument(
        "--iptables",
        action="store_true",
        help="Apply iptables rules to drop TCP RST packets",
    )
...
def main_logic():
    ...
    manager = HoneyBashManager(argv, server_args)
    manager.main()
